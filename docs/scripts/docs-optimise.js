...
class DocsAnalyzer {
  ...
  calculateCLS(content) {
    ...
    const lexicalScore = words.length > 0 ? 
      Math.min(100, (uniqueWords.size / words.length) * 150) : 0;
    
    // Topic coherence (technical term consistency)
    const technicalTerms = ['react', 'component', 'hook', 'state', 'effect', 'prop', 'typescript', 'api', 'database'];
    const technicalDensity = words.length > 0 ? 
      (words.filter(w => technicalTerms.includes(w)).length / words.length) * 100 : 0;
  ...
  analyzeDocument(filePath) {
    try {
      ...
      const codePercentage = content.length > 0 ? 
        Math.min(100, Math.round((codeContentLength / content.length) * 100)) : 0;
      ...
      const recommendations = this.generateRecommendations(clsMetrics.cls, clsTarget, codePercentage);
  ...
  async analyzeAll() {
    ...
    results.forEach(result => {
      ...
      const priority = result.cls > result.clsTarget ? 
        (result.cls - result.clsTarget > 20 ? '🔴' : result.cls - result.clsTarget > 10 ? '🟡' : '🟢') : '✅';
      
      console.log(`${statusIcon} ${relativePath} | ${result.byteSize}b | ${result.codePercentage}% code | CLS:${result.cls}/${result.clsTarget} ${priority}`);
    });
    ...
    const avgCodePercentage = Math.round(results.reduce((sum, r) => sum + r.codePercentage, 0) / totalDocs);
    
    console.log('📈 SUMMARY STATISTICS');
    console.log('====================');
    console.log(`📄 Total documents: ${totalDocs}`);
    console.log(`✅ Documents meeting CLS target: ${passedDocs}/${totalDocs} (${Math.round(passedDocs/totalDocs*100)}%)`);
  ...
  • Summary statistics`);
    ...
    if (args[0] === '--file' && args[1]) {
      ...
      if (analysis) {
        ...
        const optimizationNote = analysis.worthOptimizing ? '🔧 Worth optimizing' : '🔒 Code-dominant - limited optimization value';
        
        console.log(`📊 Analysis for ${relativePath}`);
        console.log('==============================\n');
        console.log(`${statusIcon} Status: ${analysis.cls <= analysis.clsTarget ? 'PASSED' : 'NEEDS ATTENTION'}`);
        ...
        console.log(`📊 Breakdown: Readability ${analysis.readabilityScore}, Lexical ${analysis.lexicalScore}, Coherence ${analysis.coherenceScore}`);
