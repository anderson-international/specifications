...
export function useSwipeNavigation(options: SwipeOptions): { swipeHandlers: SwipeHandlers; swipeState: SwipeState } {
  const {
    onSwipeLeft,
    onSwipeRight,
    threshold = 50,
  ...
  const handleDragMove = useCallback((clientX: number): void => {
    if (!dragging) return
    setCurrentX(clientX)
  }, [dragging])
