...
export function saveTrialDraft(key:string,userId:string,formData:Record<string,unknown>,currentStep:number):void{try{const d:TrialDraft={userId,key,formData,lastSaved:new Date().toISOString(),currentStep};localStorage.setItem(key,JSON.stringify(d));if(typeof window!=='undefined'&&typeof window.dispatchEvent==='function'&&typeof CustomEvent==='function'){window.dispatchEvent(new CustomEvent('trial-draft-saved',{detail:{key,userId}}))}}catch(e){throw new Error(`Failed to save trial draft: ${e instanceof Error?e.message:'Unknown error'}`)}}
...
export function deleteTrialDraft(key:string):void{localStorage.removeItem(key);if(typeof window!=='undefined'&&typeof window.dispatchEvent==='function'&&typeof CustomEvent==='function'){window.dispatchEvent(new CustomEvent('trial-draft-deleted',{detail:{key}}))}}
